...
- (UIImage *)dn_waterMarkNet:(NSString *)url mark:(NSString *)mark textFont:(CGFloat)textFont textColor:(UIColor *)textColor {
    ...
    //绘制水印文字
    [mark drawInRect:CGRectMake(0, 0, self.size.width, self.size.height) withAttributes:attr];
    //获取绘制后的图片
    UIImage *waterMark = UIGraphicsGetImageFromCurrentImageContext();
    //关闭图片上下文
